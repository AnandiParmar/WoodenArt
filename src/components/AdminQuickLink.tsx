...
export function AdminQuickLink() {
  ...
  useEffect(() => {
    // Check if user is admin from cookies or Redux
    const role = Cookies.get('role') || localStorage.getItem('role') || user?.role;
    const isAuthenticated = Cookies.get('isAuthenticated') === 'true' || localStorage.getItem('isAuthenticated') === 'true';
    setIsAdmin(Boolean(isAuthenticated && role === 'ADMIN'));
  }, [user]);
